...
BEGIN
RETURN
round((observed_number/base_number)*1000,decimal_places);
END;
$$ LANGUAGE plpgsql;
...
WHERE est_number ILIKE '%test%';

CREATE OR REPLACE FUNCTION insert_inspection_date()
RETURNS trigger AS
$$
BEGIN
...
CREATE TRIGGER inspection_date_update
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE insert_inspection_date();
